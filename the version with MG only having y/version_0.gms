...
SMG(Nm,i) planning state of each unit
...
*SU_eq(i,y,d,t).. SU(i,y,d,t) =g= DG_param(i,'SUC')*(II(Nm,i,y,d,t) - II(i,y,d,t-1)) ;
*SD_eq(i,y,d,t).. SD(i,y,d,t) =g= DG_param(i,'SDC')*(II(Nm,i,y,d,t-1) - II(i,y,d,t)) ;
...
ESS_init(Nm,e,y)$(ord(y) = 1).. En(Nm,e,y) =e= ES(Nm,e)*ES_param(e,'Einit') ;
ESS_balance(Nm,e,y).. En(Nm,e,y+1) =e= En(Nm,e,y) + Ce*C(Nm,e,y) - Dc(Nm,e,y)/DCe ;
ESS_min(Nm,e,y).. En(Nm,e,y) =g= ES_param(e,'Emin')*ES(Nm,e) ;
ESS_max(Nm,e,y).. En(Nm,e,y) =l= ES_param(e,'Emax')*ES(Nm,e) ;

discharge_limit(Nm,e,y).. Dc(Nm,e,y) =l= U(Nm,e,y)*ES_param(e,'PR') ;
charge_limit(Nm,e,y).. C(Nm,e,y) =l= (1 - U(Nm,e,y))*ES_param(e,'PR') ;

discharge_rate_min(Nm,e,y).. Dc(Nm,e,y) - Dc(Nm,e,y-1) =l= ES_param(e,'RDC') ;
discharge_rate_max(Nm,e,y).. Dc(Nm,e,y) - Dc(Nm,e,y-1) =g= -ES_param(e,'RDC') ;
charge_rate_min(Nm,e,y).. C(Nm,e,y) - C(Nm,e,y-1) =l= ES_param(e,'RC') ;
charge_rate_max(Nm,e,y).. C(Nm,e,y) - C(Nm,e,y-1) =g= -ES_param(e,'RC') ;

eq_Psale(Nm,y).. Psale(Nm,y) =l= 20;
eq_Ppur(Nm,y).. Ppur(Nm,y) =l= 20;
power_balance(Nm,y).. sum(i,PMG(Nm,i,y)) + sum(e,Dc(Nm,e,y) - C(Nm,e,y)) + LC(Nm,y) +
 sum(pv, Cap(pv)*Ps(y,d,t)*SII(Nm,pv,y))+ Ppur(Nm,y) =e= Pd(Nm,y) + Psale(Nm,y);

Income_eq(Nm,y).. Income(Nm,y) =e= 91.25*LMP(y)*Psale(Nm,y) ;
* SU and SD haas been deleted
OpCost_eq(Nm,y).. OpCost(Nm,y) =e= 91.25*(sum(i,DG_param(i,'MC')*II(Nm,i,y)
+ LMP(y)*Ppur(Nm,y) + DG_param(i,'m')*PMG(Nm,i,y,d,t) + LCC*LC(y,d,t)))) ;
